...
$runnerPath = Resolve-ExistingFile -PathValue (Join-Path $projectRootResolved "scripts\run_agent.ps1") -Description "Runner script"
...
Register-ScheduledTask `
  ...
  -Trigger $trigger `
  -Principal $principal `
  -Description "ARQON realtime security agent" `
  -Settings $settings `
  -Force | Out-Null

Write-Host "Startup task installed: $TaskName"
Write-Host "Task target script: $runnerPath"
